        ...
        <div class="container-fluid m-0 p-2 content">
            ...
            <div class="container-fluidm-0 my-1 p-1 bg-secondary rounded-1 text-white" style="letter-spacing: 3px;font-size:18px;">
                <div class="p-1 d-flex" style="flex-direction: row;align-items:center;justify-content:space-between">
                    ...
                    <form action="{{ route('generate-sales-report.cashier') }}" method="post" class="d-flex gap-1">
                        @csrf
...
<script>
    $(document).ready(function() {
        ...
        var mainUrl = "{{ route('fetch-sold-items.cashier') }}?today=1";
        var fetchSoldItemsUrl = "{{ route('fetch-sold-items.cashier') }}?today=1";
        let dayIdentifierValue = "(Today)";
        let orderTypeFilterSubUrl = "";
        var totalSalesUrl = "{{ route('get-sales-amount.cashier') }}?today=1";

        function fetchAllCategories() {
            $.ajax({
                type: 'get',
                url: "{{ route('fetch-categories.cashier') }}",
        ...
        todayButton.on('click', function() {
            ...
            fetchSoldItemsUrl = "{{ route('fetch-sold-items.cashier') }}?today=1";
            dayIdentifierValue = "(Today)";
            orderTypeFilterSubUrl = "";
            mainUrl = fetchSoldItemsUrl;
            fetchSoldItems();

            totalSalesUrl = "{{ route('get-sales-amount.cashier') }}?today=1";
            displayTotalSales();
        });
        ...
        yesterdayButton.on('click', function() {
            ...
            fetchSoldItemsUrl = "{{ route('fetch-sold-items.cashier') }}?yesterday=1";
            dayIdentifierValue = "(Yesterday)";
            orderTypeFilterSubUrl = "";
            mainUrl = fetchSoldItemsUrl;
            fetchSoldItems();

            totalSalesUrl = "{{ route('get-sales-amount.cashier') }}?yesterday=1";
            displayTotalSales();
        });
        ...
        dateFilterButton.on('click', function() {
            ...
            if (dateInputValue !== "") {
                fetchSoldItemsUrl = "{{ route('fetch-sold-items.cashier') }}?filter=" + dateInputValue;
                mainUrl = fetchSoldItemsUrl;
                fetchSoldItems();

                totalSalesUrl = "{{ route('get-sales-amount.cashier') }}?filter=" + dateInputValue;
                displayTotalSales();
            } else {
                $("#date_input").focus();
            }
